...
<script lang="ts">
    ...
    const searchBoards= () =>{
        ...
        const query= (): Promise<Board[]> => fetch('http://localhost:4000/boards', { signal: controller.signal, headers: { 'Authorization': `Bearer ${sessionToken}`  } }).then((res) => res.json())
    ...
    const eventListener= (e: KeyboardEvent) =>{
        ...
        query()
            .then((res) =>{
                loadingData= false;
                boards= res.filter((board) => board.title.toLocaleLowerCase().includes(e.target.value.toLocaleLowerCase()));
            }).catch(() =>{
...
<dialog id="search" bind:this={dialogSearch} class="block fixed opacity-0 translate-y-20 transition-[opacity,transform] duration-300 [&[open]]:opacity-100 [&[open]]:translate-y-0 inset-0 w-[550px] rounded-[8px] dark:bg-[#2E2E2E]">
    <div>
        <input on:keydown={(e) => handleArrowController(e)} on:keyup={(e) => eventListener(e)} autocomplete="off" type="search" name="searchBoards" id="searchBoards" placeholder="Keyword..." class="w-[430px] h-[34px] ml-[10px] outline-none text-[#BDBDBD] text-base dark:bg-[#2E2E2E]">
       
        <button on:click={dialogSearch.close()} type="button" class=" mr-[10px] rounded-md p-2 inline-flex items-center justify-center">
            ...
            {#if boards}
                <div class="p-6 divide-y divide-slate-200" >
                    {#each boards as board, index (board.id)}
                        <!-- <SearchResult {board} /> -->
                        <div id="{`item_${index}`}" class="{index === selectedIndex ? 'bg-slate-100' : ''}  flex items-center py-4 first:pt-0 last:pb-0 hover:bg-slate-100 transition-colors duration-100">
                            <img class="h-10 w-10 rounded-[6px]" src="{board.background}" alt="image of the board" />
